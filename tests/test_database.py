...
def test_dbfactory_mongo(monkeypatch):
    db = DbFactory.create_db(DbType.MONGO, "mongodb://localhost:27017/", "testdb", "testcollection")
...
def test_dbfactory_mongo_invalid():
    ...
    assert "MongoDB requires both a database name and collection name." in str(excinfo.value)
